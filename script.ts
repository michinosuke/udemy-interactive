...
let correctChoiceClassName
let incorrectChoiceClassName
...
const onClickChoice = (questionIndex, choiceIndex) => {
    ...
    if (question.isExpand) {
        ...
        question.checkboxSVGs.forEach(checkbox => {
...
// 初回のみ実行するフォーマット
const initialize = () => {
    correctChoiceClassName = [...document.querySelectorAll('div[data-purpose="answer"]')].find(div => div.className.match(/answer-result-pane--answer-correct--/)).className
    incorrectChoiceClassName = [...document.querySelectorAll('div[data-purpose="answer"]')].find(div => div.className.match(/answer-result-pane--answer-skipped--/)).className
    ...
    questions = questionElements.map((questionElement, questionIndex) => {
        ...
        // 選択肢
        const choices = [...questionElement.querySelectorAll('div')].filter(div => div.className.match(/result-pane--answer-result-pane--/))
        const choiceTexts = [...choices.map(choice => element2text(choice.querySelector('#answer-text')))]

        // チェックボックス
        const checkboxSVGs = [...choices.map(choice => choice.querySelector('div[data-purpose="answer-body"] svg use'))]
        console.log(checkboxSVGs)
        assertTrue(checkboxSVGs, 109)

        // 選択肢の要素それぞれに対して
        const answers = choices.map((choice) => choice.getElementsByClassName(correctChoiceClassName))

        const choiceContainer = [...questionElement.querySelectorAll('div')].filter(div => div.className.match(/result-pane--question-result-pane-expanded-content--/))

        // questionsに追加するオブジェクト（各プロパティの意味はこのファイル上部を参照）
        const question = {
            element: questionElement,
            checkboxSVGs,
            choiceCallbacks: choices.map((_, i) => () => onClickChoice(questionIndex, i)),
            choiceTexts,
            choiceContainer,
            choices,
            answers,
            selects: choices.map(() => false),
            questionHtml,
    ...
    // 未回答って表示が邪魔なので消す
    const statusButtons = [...document.querySelectorAll('span[data-purpose="answer-result-header-user-label"]')]
    ...
    theatreButton.addEventListener('click', () => { setTimeout(() => toggleTheatreCss(), 100) })
    toggleTheatreCss()
}
...
const reset = () => {
    questions.forEach(question => {
        ...
        question.checkboxSVGs.forEach(svg => {
